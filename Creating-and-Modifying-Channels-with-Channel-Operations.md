...
We will now discuss how you can create and modify channels.  The most common use for this is to re-reference all (or most) channels to a different reference electrode (or combination of electrodes).  A second use is to create a bipolar channel that better isolates an artifact (e.g., to isolate the vertical or horizontal EOG signals).  A third use is to replace a bad channel with interpolated values from surrounding channels (which is described in the ERPLAB User's Manual). These operations can be done either on the EEG in a dataset (ERPLAB > EEG Channel Operations) or on the ERPs in an ERPset (ERPLAB > ERP Channel Operations).
